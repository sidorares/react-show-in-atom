...
var injectClickHandler = function(port) {
  var source = null;
    ...
    for (var key in targetElement) {
      if (key.startsWith('__reactInternalInstance$')) {
        ...
        if (compInternals && e.altKey) {
          ...
          fetch(
            `http://localhost:${port}/go-to-element-source?filename=` + source.fileName + '&line=' + source.lineNumber
          );
        }
      }
    }
...
// windows: try to do something similar to above automaticly, possibly using https://technet.microsoft.com/en-us/sysinternals/dd581625.aspx
// const sock = '\\\\.\\pipe\\atom-1.18.0-beta0-YXNpZG9ybzE=-x64-sock'
...
app.get('/open-in-editor.js', (req, res) => {
  res.end(`(${injectClickHandler.toString()})(${port})`);
});
